...
function ManageUserInterfaceUserInitialEvaluationInformation() {
	...
	useEffect(() => {
		if (isLanguageReady) {
			axiosInstance
				.post("/api/Admin/GetUserInformation", { userId: userId }) // Call the API to get user data
				.then((response) => {
					const data = response.data;
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			try {
				...
				await axiosInstance.post("/api/Admin/UpdateUserInformation", values);
				alert(
					language === "fa"
